'use strict'
/* globals value, state */
/* eslint-disable no-native-reassign, no-extend-native, no-new-func */

const common = require('./common')

const arrayFunction = function () {
  // states if the value is an array
  const array = function () {
    array.validators = []
    array.validators.push({
      function: function isArray () {
        state = state && Array.isArray(value)
      },
      parameters: {}
    })
    return array
  }

  // states if the array.length is higher than the passed value
  array.max = function max (max) {
    this.validators.push({
      function: function maxArray () {
        state = state && value.length <= max
      },
      parameters: {
        max: max
      }
    })
    return this
  }

  // states if the array.length is lower than the passed value
  array.min = function min (min) {
    this.validators.push({
      function: function minArray () {
        state = state && value.length >= min
      },
      parameters: {
        min: min
      }
    })
    return this
  }

  // Makes writable the length object property
  Object.defineProperties(array, {
    'length': {
      writable: true
    }
  })
  // states if the array.length is the same if the passed value
  array.length = function length (len) {
    this.validators.push({
      function: function lengthArray () {
        state = state && value.length === length
      },
      parameters: {
        length: len
      }
    })
    return this
  }

  // states if the array value contains the passed value
  array.contains = function contains (containValue) {
    this.validators.push({
      function: function contains () {
        state = state && value.indexOf(containValue) !== -1
      },
      parameters: {
        containValue: containValue
      }
    })
    return this
  }

  // Extends an array function
  array.extend = function (func) {
    return common.extend(this, func)
  }

  array.toFunction = function () {
    return common.toFunction(this.validators)
  }

  return array
}

module.exports = arrayFunction()
